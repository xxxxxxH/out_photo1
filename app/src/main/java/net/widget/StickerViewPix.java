...
public class StickerViewPix extends FrameLayout {
    ...
    private BitmapStickerIconPixel mDeleteIcon;
    private BitmapStickerIconPixel mZoomIcon;
    private BitmapStickerIconPixel mFlipIcon;
    ...
    private List<StickerPixel> mStickers = new ArrayList<> ();
    private StickerPixel mHandlingSticker;
    ...
    public StickerViewPix(Context context) {
        this(context, null);
        this.context = context;
    }

    public StickerViewPix(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        this.context = context;
    }

    public StickerViewPix(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDeleteIcon = new BitmapStickerIconPixel(ContextCompat.getDrawable(getContext(), R.mipmap.ic_close_white_18dp));
        mZoomIcon = new BitmapStickerIconPixel(ContextCompat.getDrawable(getContext(), R.mipmap.ic_scale_white_18dp));
        mFlipIcon = new BitmapStickerIconPixel(ContextCompat.getDrawable(getContext(), R.mipmap.ic_flip_white_18dp));
    }
    ...
    private void drawStickers(Canvas canvas) {
        for (int i = 0; i < mStickers.size(); i++) {
            StickerPixel sticker = mStickers.get(i);
            if (sticker != null) {
                sticker.draw(canvas);
            }
        }
...
//            mFlipIcon.draw(canvas, mBorderPaint);
    ...
    private void configIconMatrix(BitmapStickerIconPixel icon, float x, float y, float rotation) {
        icon.setX(x);
        icon.setY(y);
...
//                Log.e(TAG, "Module ACTION_DOWN");
                ...
                if (mHandlingSticker != null) {
                    ...
                    Log.e("StickerPixel getTag", mHandlingSticker.getTag());
...
//                Log.e(TAG, "Module ACTION_UP");
                ...
                if (mCurrentMode == ActionMode.DELETE && mHandlingSticker != null) {
                    ...
                    if (mHandlingSticker.getTag().toString().equalsIgnoreCase("frame")) {
                        Share.isFrameAvailable = false;
//                        PixelEffectActivity.drawables_sticker.remove(Share.STICKER_POSITION);
                    }
...
//                    Share.y = event.getY();
    ...
    private boolean checkIconTouched(BitmapStickerIconPixel icon) {
    ...
    /**
     * find the touched StickerPixel
     **/
    private StickerPixel findHandlingSticker() {
        ...
        for (int i = mStickers.size() - 1; i >= 0; i--) {
            if (isInStickerArea(mStickers.get(i), mDownX, mDownY)) {
                ...
                Log.e("StickerPixel Touch", "findHandlingSticker else");
                Share.IsSelectFrame = false;
            }
        }
        return null;
    ...
    private StickerPixel findHandlingSticker2() {
        Log.e("TAG Module", "findHandlingSticker");
        for (int i = mStickers.size() - 1; i >= 0; i--) {
            Log.e("TAG Module", "findHandlingSticker if");
            return mStickers.get(i);
    ...
    private boolean isInStickerArea(StickerPixel sticker, float downX, float downY) {
    ...
    /**
     * StickerPixel's drawable will be too bigger or smaller
     ...
     **/
    private void transformSticker(StickerPixel sticker) {
        if (sticker == null) {
    ...
    public void replace(StickerPixel sticker) {
        replace(sticker, true);
    }

    public void replace(StickerPixel sticker, boolean needStayState) {
    ...
    public void addSticker(StickerPixel sticker) {
        if (sticker == null) {
            Log.e(TAG, "StickerPixel to be added is null!");
            return;
        }
        float offsetX = (getWidth() - sticker.getWidth()) / 2;
        float offsetY = (getHeight() - sticker.getHeight()) / 2;
    ...
    public float[] getStickerPoints(StickerPixel sticker) {
    ...
    public BitmapStickerIconPixel getFlipIcon() {
        return mFlipIcon;
    }

    public void setFlipIcon(BitmapStickerIconPixel flipIcon) {
        mFlipIcon = flipIcon;
        postInvalidate();
    }

    public BitmapStickerIconPixel getZoomIcon() {
        return mZoomIcon;
    }

    public void setZoomIcon(BitmapStickerIconPixel zoomIcon) {
        mZoomIcon = zoomIcon;
        postInvalidate();
    }

    public BitmapStickerIconPixel getDeleteIcon() {
        return mDeleteIcon;
    }

    public void setDeleteIcon(BitmapStickerIconPixel deleteIcon) {
    ...
    public interface OnStickerOperationListener {
        void onStickerClicked(StickerPixel sticker);

        void onStickerDeleted(StickerPixel sticker);

        void onStickerDragFinished(StickerPixel sticker);

        void onStickerZoomFinished(StickerPixel sticker);

        void onStickerFlipped(StickerPixel sticker);
    }
}
