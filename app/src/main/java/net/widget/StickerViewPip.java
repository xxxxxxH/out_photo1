...
import net.basicmodel.PhotoDisplayPipActivity;
...
public class StickerViewPip extends FrameLayout {
    ...
    private BitmapStickerIconPip mDeleteIcon;
    private BitmapStickerIconPip mZoomIcon;
    private BitmapStickerIconPip mFlipIcon;
    ...
    public static List<StickerPip> mStickers = new ArrayList<> ();
    public static StickerPip mHandlingSticker;
    ...
    public static List<StickerPip> drawable_list = new ArrayList<> ();
    ...
    public StickerViewPip(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        this.context = context;
    }

    public StickerViewPip(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDeleteIcon = new BitmapStickerIconPip(ContextCompat.getDrawable(getContext(), R.mipmap.ic_close_white_18dp));
        mZoomIcon = new BitmapStickerIconPip(ContextCompat.getDrawable(getContext(), R.mipmap.ic_scale_white_18dp));
        mFlipIcon = new BitmapStickerIconPip(ContextCompat.getDrawable(getContext(), R.mipmap.ic_flip_white_18dp));
    }
    ...
    private void drawStickers(Canvas canvas) {
        for (int i = 0; i < mStickers.size(); i++) {
            StickerPip sticker = mStickers.get(i);
            if (sticker != null) {
                sticker.draw(canvas);
            }
        }
...
//            mFlipIcon.draw(canvas, mBorderPaint);
    ...
    private void configIconMatrix(BitmapStickerIconPip icon, float x, float y, float rotation) {
        icon.setX(x);
        icon.setY(y);
...
//                    Share.y = event.getY();
    ...
    private boolean checkIconTouched(BitmapStickerIconPip icon) {
    ...
    /**
     * find the touched StickerPip
     **/
    private StickerPip findHandlingSticker() {
    ...
    private StickerPip findHandlingSticker2() {
        Log.e("TAG Module", "findHandlingSticker");
        for (int i = mStickers.size() - 1; i >= 0; i--) {
            Log.e("TAG Module", "findHandlingSticker if");
            return mStickers.get(i);
    ...
    private boolean isInStickerArea(StickerPip sticker, float downX, float downY) {
    ...
    /**
     * StickerPip's drawable will be too bigger or smaller
     ...
     **/
    private void transformSticker(StickerPip sticker) {
        if (sticker == null) {
    ...
    public void replace(StickerPip sticker) {
        replace(sticker, true);
    }

    public void replace(StickerPip sticker, boolean needStayState) {
    ...
    public void addSticker(StickerPip sticker) {
        if (sticker == null) {
            Log.e(TAG, "StickerPip to be added is null!");
            return;
        }
        float offsetX = (getWidth() - sticker.getWidth()) / 2;
        float offsetY = (getHeight() - sticker.getHeight()) / 2;
    ...
    public float[] getStickerPoints(StickerPip sticker) {
        if (sticker == null) return new float[8];
        return sticker.getMappedBoundPoints();
    }

    public void save(File file) {
        StickerUtilsPip.saveImageToGallery(file, createBitmap());
        StickerUtilsPip.notifySystemGallery(getContext(), file);
    }
    ...
    public BitmapStickerIconPip getFlipIcon() {
        return mFlipIcon;
    }

    public void setFlipIcon(BitmapStickerIconPip flipIcon) {
        mFlipIcon = flipIcon;
        postInvalidate();
    }

    public BitmapStickerIconPip getZoomIcon() {
        return mZoomIcon;
    }

    public void setZoomIcon(BitmapStickerIconPip zoomIcon) {
        mZoomIcon = zoomIcon;
        postInvalidate();
    }

    public BitmapStickerIconPip getDeleteIcon() {
        return mDeleteIcon;
    }

    public void setDeleteIcon(BitmapStickerIconPip deleteIcon) {
    ...
    public interface OnStickerOperationListener {
        void onStickerClicked(StickerPip sticker);

        void onStickerDeleted(StickerPip sticker);

        void onStickerDragFinished(StickerPip sticker);

        void onStickerZoomFinished(StickerPip sticker);

        void onStickerFlipped(StickerPip sticker);
    }
}
